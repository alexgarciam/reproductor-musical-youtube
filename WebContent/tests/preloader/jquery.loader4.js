...
( function($) {
	var jQueryLoaderPluginSettings = {
		...
		'bgOpacity' :0.85,
		'load' : [],
		'message' :'Cargando los elementos: %f (%p% | %c de %t)',
		'ending_message' :'Una vez cargado ya no será cargado de nuevo.',
		'fadeOutSpeed' :'slow'
	};
	var jQueryLoaderPluginMethods = {
		...
		show : function(l) {
			for ( var el in l) {
				if ($.isArray(l[el])) {
					...
					if (c == 0) {
						...
						$('<div></div>').attr('id', i + 'loader').attr('rel',
								l.length).attr('loaded', 0).css( {
							width :'0',
							height :'1px',
					...
					case 'css':
						...
						break;
					case 'html':
						$('<div></div>').attr('rel', l[el][2]).load(
								l[el][0],
								function() {
									$($(this).attr('rel')).append(
											$(this).html());
									jQueryLoaderPluginMethods.show(l)
								});
						break;
			...
			$('#' + i + 'loaderText').text(
					jQueryLoaderPluginSettings.ending_message);
			$('#' + i + 'loader')
					.animate(
							{
								width :'100%'
							...
							function() {
								...
								$(
										'#' + i + 'loader, #' + i + ', #' + i
												+ 'loaderText')
										.fadeOut(
												jQueryLoaderPluginSettings.fadeOutSpeed)
							})
		},
		update : function(f) {
			i = 'preLoadGUIBeta3';
			...
			msg = msg.replace(/(%f)|(%p)|(%c)|(%t)/g, function(m) {
				return (m == '%f') ? f : ((m == '%p') ? (Math.round(ld / t
						* 100)) : ((m == '%c') ? ld : t))
			});
